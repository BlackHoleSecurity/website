...
class Blog_model extends CI_Model
{
    ...
    {
        $query = $this->db
            ->query("SELECT tbl_post.*,user_name,COUNT(comment_id) AS comment_total,tbl_category.* FROM tbl_post 
    ...
    {
        $query = $this->db
            ->query("SELECT * FROM tbl_post LEFT JOIN tbl_user ON post_user_id=user_id 
    ...
    function count_views($kode)
    {
        ...
        $cek_ip = $this->db->query(
            "SELECT * FROM tbl_post_views WHERE view_ip='$user_ip' AND view_post_id='$kode' AND DATE(view_date)=CURDATE()"
        );
        if ($cek_ip->num_rows() <= 0) {
            $this->db->trans_start();
            $this->db->query(
                "INSERT INTO tbl_post_views (view_ip,view_post_id) VALUES('$user_ip','$kode')"
            );
            $this->db->query(
                "UPDATE tbl_post SET post_views=post_views+1 where post_id='$kode'"
            );
    ...
    {
        $query = $this->db->query(
            "SELECT * FROM tbl_comment WHERE comment_post_id='$kode' AND comment_status='1' AND comment_parent='0'"
        );
        return $query;
    }
