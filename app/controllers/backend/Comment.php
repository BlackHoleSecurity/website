...
class Comment extends CI_Controller
{
    ...
    function index()
    {
        $count = $this->db->get_where('tbl_comment', array(
            'comment_parent' => '0'
        ));
        ...
        $data['total_unpublish'] = $this->db
            ->get_where('tbl_comment', array('comment_status' => '0'))
            ->num_rows();
    ...
    function reply()
    {
        ...
        $comment_id = htmlspecialchars(
            $this->input->post('comment_id', true),
            ENT_QUOTES
        );
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->comment_model->reply_comment(
                $post_id,
                $comment_id,
                $comments,
                $user_id,
    ...
    {
        $comment_id = htmlspecialchars(
            $this->input->post('comment_id4', true),
            ENT_QUOTES
        );
    ...
    {
        $comment_id = htmlspecialchars(
            $this->input->post('comment_id2', true),
            ENT_QUOTES
        );
    ...
    {
        $comment_id = htmlspecialchars(
            $this->input->post('comment_id3', true),
            ENT_QUOTES
        );
    ...
    function change()
    {
        if (isset($_FILES["file"]["name"])) {
            ...
            if (!$this->upload->do_upload('file')) {
                ...
                $id = htmlspecialchars(
                    $this->input->post('comment_id5', true),
                    ENT_QUOTES
                );
    ...
    function unpublish()
    {
        ...
        $this->pagination->initialize($config);
        $data['page'] = $this->pagination->create_links();
        $data['data'] = $this->comment_model->get_all_comment_unpublish(
            $offset,
            $limit
        );
        $data['total_rows'] = $count->num_rows();
        $data['total_all'] = $this->db
            ->get_where('tbl_comment', array('comment_parent' => '0'))
            ->num_rows();
